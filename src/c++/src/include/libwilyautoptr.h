...
//        LOG(INFO)<<"the memory of ["<<abi::__cxa_demangle(typeid(rp).name(), nullptr,nullptr, nullptr)<<"] address is "<<&mutex;
        pthread_mutex_lock(&mutex);
        --rp->count;
        pthread_mutex_unlock(&mutex);
        if (rp->count == 0) {
            LOG(INFO)<<"the memory of ["<<abi::__cxa_demangle(typeid(rp).name(), nullptr,nullptr, nullptr)<<"] will be delete";
            delete rp;
        } else if (rp->count < 0) {
