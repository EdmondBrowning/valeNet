...
namespace wily {
    ...
    void VSHA1::Transform(UINT_32 *pState, const UINT_8 *pBuffer) {
    ...
    void VSHA1::Update(const UINT_8 *pbData, UINT_32 uLen) {
    ...
    bool VSHA1::HashFile(const TCHAR *tszFileName) {
    ...
    bool VSHA1::ReportHash(TCHAR *tszReport, REPORT_TYPE rtReportType) const {
    ...
    bool VSHA1::ReportHashStl(std::basic_string<TCHAR> &strOut, REPORT_TYPE rtReportType) const {
        TCHAR tszOut[84];
    ...
    bool VSHA1::GetHash(UINT_8 *pbDest20) const {
        if (pbDest20 == NULL) return false;
        memcpy(pbDest20, m_digest, 20);
        return true;
    }

    std::bitset<160> VSHA1::getSHA1() {
        unsigned char chSha1[20] = "";
        GetHash(chSha1);
        std::bitset<160> bits;
        for (int i = 0; i < 20; ++i)
            for (int j = 0; j < 8; ++j)
                bits[i * 8 + j] = (chSha1[i] >> j & 1);
        return bits;
    }

    std::bitset<160> VSHA1::getSHA1(char *s) {
        Update((unsigned char *) s, strlen(s));
        Final();
        return getSHA1();
    }

    std::bitset<160> VSHA1::getSHA1(std::string s) {
        char* chSha1 = (char *) s.data();
        return getSHA1(chSha1);
    }
}
#pragma warning(pop)